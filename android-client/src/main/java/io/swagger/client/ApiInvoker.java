...
public class ApiInvoker {
  ...
  //NEW CODE - add this method to this class
  public void useModifiedTrustManager(Context c){
    stopQueue();
    initConnectionRequestModedTrustManager(c);
  }
  ...
  */
  ...
  public static void initializeInstance(Cache cache) {
    ...
    //NEW CODE - change the connectionTimeout value from 30 to 5
    //initializeInstance(cache, null, 0, null, 30);
    initializeInstance(cache, null, 1, null, 5);
  }
  ...
  public static void initializeInstance(Cache cache, Network network, int threadPoolSize, ResponseDelivery delivery, int connectionTimeout) {
    ...
    INSTANCE.authentications.put("key", new ApiKeyAuth("query", "key"));
      ...
      request = new GetRequest(url, headers, null, stringRequest, errorListener);
       ...
       if (formParamStr != null) {
          ...
          request = new PostRequest(url, headers, null, (HttpEntity) body, stringRequest, errorListener);
            ...
            // NEW CODE -  had to modify the contentType of the request a little bit in order to get POST requests to work.  Previously, the request had 2 contentType headers, and the server did not like dat
           //request = new PostRequest(url, headers, contentType, new StringEntity(serialize(body), "UTF-8"), stringRequest, errorListener);
           StringEntity mStringEntity = new StringEntity(serialize(body), "UTF-8");
           mStringEntity.setContentType(contentType);
           request = new PostRequest(url, headers, null, mStringEntity, stringRequest, errorListener);
          }
       } else {
  ...
  //NEW CODE - add this method to this class
  // this method creates a new RequestQueue that uses our custom trustmanager which contains our prog-dev and prog-stage certificates
  private void initConnectionRequestModedTrustManager(Context mContext) {
  try{

      mRequestQueue = Volley
              .newRequestQueue(mContext,
                      new HurlStack(null, TrustManagerHelper.getCustomSSLSocketFactory(mContext)));
      mRequestQueue.start();
